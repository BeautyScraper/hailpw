...
def random_line(npath):
    if not Path(npath).is_absolute():
        newpath = Path('files') /  Path(npath)
    else:
        newpath = Path(npath)
        if newpath.is_file():
            return randomLine_helper(str(newpath.with_suffix('.txt')))[0] , None
...
if __name__ == '__main__':
    ...
    line = random_line(r"C:\Work\OneDrive - Creative Arts Education Society\Desktop\rarely\G1\to_video\wan\Father_sleeping.txt")[0]
    print(line)
