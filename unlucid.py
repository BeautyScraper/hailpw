...
def create_image(page):
    page.goto("https://unlucid.ai/create")
    prompt, _ = random_line('unlucid.txt')
    img_dir = Path(r"C:\Work\OneDrive - Creative Arts Education Society\Desktop\rarely\G1\lucid_faces")
    sel_img = random_image_from_dir(img_dir)
    if not sel_img:
        return
    print(f"Using image: {sel_img}")
    page.locator('input[type="file"]').nth(1).set_input_files(str(sel_img))
...
def create_video(page):
    ...
    page.locator("#input").fill(prompt)
    page.locator('input[type="file"]').nth(0).set_input_files(str(sel_img))
    page.locator("button.bg-primary:nth-child(5)").click()
    page.locator("#consent").click()
    ...
    # page.get_by_role("button", name="4").click()
    sleep(5)
    statuselem = page.locator("p.text-white:nth-child(3)")
    try:
        status = statuselem.inner_text()
    except:
        breakpoint()
    while True:
        try:
            if status != statuselem.inner_text():
                status = statuselem.inner_text()
...
def download_videos(page):
    ...
    for btn4 in  page.locator('button[variant="ghost"]').all():
        ...
        rand_filename = f"unlucid_{int(time())}.{extension}"
        #create 
        download.save_as(os.path.join(download_path, rand_filename))
